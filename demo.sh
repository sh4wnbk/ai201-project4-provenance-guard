...
VERIFY=$(jq -nc --arg c "demo-cert" --arg t "$FORMAL_HUMAN" --arg a "I wrote this myself. I am an economist and this is my professional writing style." '{creator_id:$c, sample_text:$t, attestation:$a}')
curl -s -X POST "$BASE/verify" -H "Content-Type: application/json" -d "$VERIFY" | jq .
echo

echo "=== 8. Reviewer approves the verification ==="
REVIEW=$(jq -nc --arg c "demo-cert" '{creator_id:$c, approve:true}')
curl -s -X POST "$BASE/verify/review" -H "Content-Type: application/json" -d "$REVIEW" | jq .
echo

echo "=== 9. Verified creator submits — detection still runs, certificate appears alongside ==="
submit "$FORMAL_HUMAN" demo-cert | jq '{label_key,label,certificate}'
echo
